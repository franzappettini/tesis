...
def absDiffList(storedDataV, diffStep):
    print("lista", storedDataV)
    print("len lista = ",len(storedDataV))
    print("last lista =",storedDataV[-1])
    if len(storedDataV) - diffStep < 0:
        return abs(storedDataV[-1])
    else:
        return abs(storedDataV[-1] - storedDataV[len(storedDataV) -1 - diffStep])    
...
def readdaq():
    with nidaqmx.Task() as task:
        ...
        value.append(moving_averageV(absDiffList(storedDataV,1), 200, E))
        print(value[4])
    return value
